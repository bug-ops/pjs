...
- **BREAKING** `pjson-rs`'s `SimdJsonProcessor` (`infrastructure::integration::simd_acceleration`) — a standalone `sonic_rs::from_slice`-based JSON validator/parser (`validate_json`, `extract_priority_field`, `validate_batch`) with zero production call sites anywhere in `pjs-core`, `pjs-wasm`, or `pjs-demo`; only its own unit tests and the crate's integration test suite referenced it. The real JSON validate/parse path is `parser::SonicParser`/`parser::SimpleParser`, unaffected by this removal. `SimdStreamProcessor` (also in `simd_acceleration.rs`) is a separate, unrelated type used on the live SSE path (`streaming_helpers::default_sse_response`) and is likewise unaffected — despite sharing a file, the two SIMD types never called each other at runtime. The #456/#462 depth-bypass hardening previously applied to `SimdJsonProcessor::validate_json`/`extract_priority_field` (see the `### Security` entry above) goes away with the type, but the production guard it mirrored is untouched: `SecurityValidator::validate_json_depth_bytes` is still enforced on the real, live JSON entry points (`parser::SonicParser`/`parser::SimpleParser`), both regression-tested for the same unbounded-depth class of input, so no security coverage is lost. **Breaking:** `pjson_rs::infrastructure::integration::simd_acceleration::SimdJsonProcessor` no longer exists; use `parser::SonicParser`/`parser::SimpleParser` for JSON validation and parsing instead (#468)
