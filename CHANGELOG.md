...
- **BREAKING** `JsonData`'s `Deserialize` impl (`pjson-rs-domain`'s `value_objects::json_data::JsonDataVisitor`) recursed through `visit_seq`/`visit_map` with no depth bound, so nesting depth in the input mapped one-to-one onto stack frames (CWE-674) and onto retained per-level preallocations (CWE-789). Confirmed exploitable via `rmp-serde`: 65 bytes of MessagePack (nested `fixarray`-of-1 headers) recursed unbounded and aborted the process with `fatal runtime error: stack overflow` (not a catchable `Result::Err`) at 100k levels, even on the main thread; nested `array32`/`map32` headers retained memory proportional to `depth * cautious_capacity`'s per-level preallocation cap (introduced in #465, roughly 1 MiB/level for arrays, more for objects once `HashMap`'s bucket-table overhead is accounted for) before any depth guard existed. `JsonData::deserialize` now threads nesting depth through a `DeserializeSeed`-based `JsonDataSeed` (not a thread-local — `Deserialize::deserialize` takes no state, so a nested `next_element()`/`next_entry()` would otherwise restart at depth 0), rejecting once depth reaches a new `MAX_DESERIALIZE_DEPTH` (64, matching `pjson_rs::config::security::JsonLimits::max_depth`'s default; the constant cannot be shared with `pjson_rs::config::security::JsonLimits::max_depth` itself since the dependency direction is `pjs-core` -> `pjs-domain` — it is `pub`, re-exported from both `pjson_rs_domain::value_objects` and the crate root, so callers can keep their own limits in sync with it). `visit_map` now reads keys/values via `next_key::<String>()` + `next_value_seed(...)` instead of `next_entry()`, which cannot carry a seed for the value; `visit_some` forwards `self` unchanged, so `Option` wrapping does not consume a depth level (correct, since `Some(x)` is not itself a container) — this leaves `visit_some` as the one recursive entry point the counter does not track, safe only because none of `serde_json`, `rmp-serde`, or common CBOR deserializers ever invoke it from `deserialize_any`. **Breaking:** JSON or MessagePack input nested deeper than 64 levels that previously deserialized successfully into `JsonData` now fails with a "JSON nesting depth exceeds maximum of 64" error; pre-1.0, no deprecation cycle (#464)
- `Schema`'s `Deserialize` impl (`pjson-rs-domain`'s `value_objects::schema::Schema`) was a plain `#[derive(Deserialize)]` with no recursion-depth bound, even though `Schema` is a recursive enum (`Array::items: Box<Schema>`, `Object::properties: HashMap<String, Schema>`, `OneOf`/`AllOf::schemas: SmallVec<[Box<Schema>; 4]>`) — the same CWE-674 class as `JsonData` above, and the same in-tree `serde_json`-only mitigation gap. Reuses the `MAX_DESERIALIZE_DEPTH` constant from the `JsonData` fix above (same crate, so no need for a second constant of the same value). Because `Schema`'s `Deserialize` is `#[derive]`d rather than a hand-written `Visitor`, it can't thread a `DeserializeSeed` through derive-generated field deserialization the way `JsonData` does; depth is instead tracked with a thread-local counter, incremented by a `deserialize_with` on each of the three recursive fields and released by an RAII guard on drop (restoring correctly even when deserialization errors or unwinds). `Array::items` additionally needed `#[serde(default, ...)]` alongside its new `deserialize_with`, since `deserialize_with` alone suppresses serde derive's implicit "absent `Option<T>` field -> `None`" behavior — without it, any document omitting `items` (previously valid) would newly fail with `missing field 'items'`; `#[serde(default, ...)]` restores the pre-fix optional behavior. Exceeding the depth bound now returns a deserialization error instead of recursing further, but only for formats with no competing recursion limit of their own (MessagePack, CBOR, etc.) — over `serde_json`, `Schema`'s externally-tagged wire encoding costs 2-3 JSON container tokens per nesting level (varies by variant), so `serde_json`'s own ~128-level structural limit is what actually fires, at a `Schema` depth as low as ~43 for `Object`-heavy nesting and by depth 64 for `Array`-only nesting, always at or before this guard's own check. **Breaking:** a `Schema` document nested deep enough to exceed 64 levels via a format with no recursion limit of its own (e.g. MessagePack) now fails with a "Schema nesting depth exceeds maximum of 64" error instead of exhausting the stack; over JSON specifically, sufficiently deep nesting already failed before this fix too, via `serde_json`'s own recursion-limit error rather than this one; pre-1.0, no deprecation cycle (#470)
- **BREAKING** `SchemaDefinitionDto`'s `Deserialize` impl (`pjson-rs`'s `application::dto::schema_dto::SchemaDefinitionDto`) was a plain `#[derive(Deserialize)]` recursive enum (`Array::items: Box<SchemaDefinitionDto>`, `Object::properties: HashMap<String, SchemaDefinitionDto>`, `OneOf`/`AllOf::schemas: Vec<SchemaDefinitionDto>`) with no recursion-depth bound — the same CWE-674 class `Schema` was fixed for in #470, except `SchemaDefinitionDto` also recurses (via `impl From<SchemaDefinitionDto> for Schema`) only *after* deserialization has already fully materialized the DTO tree, so `Schema`'s own guard never got a chance to bound the DTO's own deserialization. Porting #470's guard pattern verbatim (thread-local counter + `deserialize_with`, reusing `pjson_rs_domain::MAX_DESERIALIZE_DEPTH`) was not sufficient on its own: `SchemaDefinitionDto` used serde's internally-tagged representation (`#[serde(tag = "type", rename_all = "lowercase")]`, e.g. `{"type": "string", ...}`), and internally-tagged enums buffer the whole nested value into a generic `Content` tree to find the tag *before* any field's `deserialize_with` hook runs — so for a self-describing format with no recursion limit of its own (confirmed via `rmp-serde`, mirroring #470's own confirmation format), a sufficiently deep payload (reproduced at 200k levels) still overflowed the stack during that buffering pass, regardless of the guard. `SchemaDefinitionDto` now uses serde's default externally-tagged representation (e.g. `{"Array": {"items": ..., ...}}`, matching `Schema`'s own tagging style) instead, which lets the `deserialize_with` guard run on every recursive field as it's reached, the same as `Schema`'s fix; `Array::items` needed the same `#[serde(default, ...)]` addition as `Schema::items` did, for the same reason. Exceeding the depth bound now returns a deserialization error instead of recursing further, but only for formats with no competing recursion limit of their own (MessagePack, CBOR, etc.) — over `serde_json`, this externally-tagged `Array` encoding costs 2 JSON container tokens per nesting level (a tag-wrapper object plus the struct's own object), so `serde_json`'s own ~128-level structural limit is what actually fires first: empirically confirmed, depth `MAX_DESERIALIZE_DEPTH - 1` (63) deserializes successfully over JSON, while depth `MAX_DESERIALIZE_DEPTH` (64) already fails with `serde_json`'s own "recursion limit exceeded" error, at or before this guard's own check. **Breaking:** the wire format of every `SchemaDefinitionDto` variant changes from internally-tagged (`{"type": "string", "min_length": ...}`) to externally-tagged (`{"String": {"min_length": ...}}`), affecting `SchemaRegistrationDto::schema` and any other JSON payload containing a `SchemaDefinitionDto`; a document nested deep enough to exceed 64 levels via a format with no recursion limit of its own (e.g. MessagePack) now fails with a "SchemaDefinitionDto nesting depth exceeds maximum of 64" error instead of exhausting the stack; over JSON specifically, sufficiently deep nesting already failed before this fix too, via `serde_json`'s own recursion-limit error rather than this one; pre-1.0, no deprecation cycle (#476)
- `SonicParser::pre_validate_json_bytes` (`crates/pjs-core/src/parser/sonic.rs`) tracked JSON nesting depth by counting `{`/`[`/`}`/`]` bytes across the whole input with no in-string state, so a crafted payload with unmatched bracket characters inside string literals (e.g. `["]",["]",["]", ...,1]]]...`) could report a shallow depth while its real structural depth was unbounded — `sonic-rs` 0.5.8 has no internal recursion limit, so the bypassed payload then recursed in `sonic_rs::from_slice` until the process stack was exhausted (`fatal runtime error: stack overflow`, SIGABRT — not a catchable `Result::Err`) (CWE-674). The scan now tracks in-string state (`"` toggles it, `\` escapes the following byte) via a new shared `SecurityValidator::validate_json_depth_bytes`, so delimiter bytes inside strings are never counted as structural nesting. The same gap existed in two other call sites that reach `sonic_rs`/`serde_json` deserialization with no depth guard at all: `Parser::parse`'s fallback to `SimpleParser` on any `SonicParser` error (`crates/pjs-core/src/parser/mod.rs`) — previously unbounded regardless of how the sonic-side check behaved, since `SimpleParser::parse` (`crates/pjs-core/src/parser/simple.rs`) had no depth validation of its own — and `SimdJsonProcessor::validate_json`/`extract_priority_field` (`crates/pjs-core/src/infrastructure/integration/simd_acceleration.rs`), a separate public `sonic_rs::from_slice` entry point, reproducibly stack-overflowable at depth 50,000 with plain nesting, no string trick required. Both now call the same shared validator before parsing. **Behavior change:** `SimpleParser::parse` previously had no explicit depth check of its own and relied on `serde_json`'s internal ~128-level recursion guard as an incidental backstop; it now enforces `SecurityConfig.json.max_depth` (default 64) directly, so input nested between 65 and 127 levels that previously parsed successfully through this path now correctly fails with a depth-exceeded error, matching the sonic path's behavior. `SecurityConfig::validate()` also now rejects `json.max_depth` above a new `MAX_SAFE_JSON_DEPTH` (512) hard ceiling, so a misconfigured (e.g. deserialized) config cannot reopen this class of bug by setting an unreasonably large limit — `sonic-rs`'s missing recursion guard means `max_depth` is the only defense, so validating it is now part of config sanity-checking, not just a >0 check. (Note: `SonicParser`, `SimpleParser`, and `Parser` referenced throughout this entry are subsequently removed later in this same release — see `### Removed` below; the shared `SecurityValidator::validate_json_depth_bytes` guard this entry introduced is retained, but its parser callers are not.) (#456)
...
- `ClientMetrics::recommended_frame_delay` (`infrastructure::websocket`) now clamps its computed delay to a new `MAX_ADAPTIVE_FRAME_DELAY` (1s) constant. This method's output was newly wired into `AdaptiveStreamController::stream_frames`'s per-frame `sleep` in this same PR (see Changed below) but was left unbounded: `average_processing_time_ms` is derived from client-supplied `processing_time_ms` in `FrameAck` with no upper-bound validation anywhere in the ingest path, so a single malicious ack (e.g. `processing_time_ms: 100_000_000_000`) could drive the per-frame sleep to roughly 46 days and stall that session's stream indefinitely (#431)
- `JsonDataVisitor::visit_seq`/`visit_map` (`pjson-rs-domain`, `value_objects::json_data`, added in #455) preallocated `Vec`/`HashMap` capacity directly from the deserializer-supplied `SeqAccess`/`MapAccess::size_hint()`, with no upper bound. For length-prefixed self-describing formats (MessagePack, CBOR, etc.) this hint is read from untrusted input before any element is validated, so a payload of a few bytes claiming e.g. `u32::MAX` elements could trigger an attempted multi-gigabyte allocation in a single call (CWE-789). `serde_json`'s `SeqAccess`/`MapAccess` never populate `size_hint()`, so the shipped HTTP ingestion path was unaffected, but `JsonData::deserialize` is public API on `pjson-rs-domain` documented as working with any self-describing format via `deserialize_any`. A new private `cautious_capacity` helper now caps a single preallocation request to at most 1 MiB worth of *elements* (`capacity * size_of::<Element>() <= 1 MiB`), mirroring the bound `serde`'s own container `Deserialize` impls apply via `serde::__private::size_hint::cautious` (not a stable path this crate can depend on directly, so reimplemented locally); note this bounds the requested element count, not the resulting allocation's actual size — `HashMap::with_capacity` in particular can still allocate a larger backing table once load-factor and bucket rounding are applied (~2.65 MiB observed for a 1 MiB-worth-of-elements capacity request), same headroom `serde`'s own cap has. This closes the single-collection, single-shot amplification (an attacker-controlled hint on one `Vec`/`HashMap`); it does not add a recursion-depth guard, so deeply nested collections in a non-JSON format can still retain multiple such allocations across nesting levels — tracked separately (#461)
- **Partial mitigation of #515** (`infrastructure::http::axum_adapter`'s shared middleware stack, `apply_common_layers`, applied by every `create_pjs_router*` variant) — this PR does not fully close #515; a follow-up issue tracks the remaining gap described below. Previously there was no request timeout and no concurrency cap: a stalled handler could hold server resources open indefinitely, and `RateLimitMiddleware`'s per-client token bucket does nothing to bound total concurrent connections across all clients (CWE-400). Three new layers now address the parts reachable from `pjs-core` (which only builds `Router`s — it never owns the `TcpListener`/`axum::serve` call, that's `pjs-demo`): `tower::limit::GlobalConcurrencyLimitLayer` (512) caps concurrent in-flight request handling with one process-wide `Arc<Semaphore>` — deliberately not the plain `ConcurrencyLimitLayer`, which axum's per-route `Router::layer` application would have turned into an independent semaphore per route (an effective cap of `512 * route_count`, not a real global one); `tower_http::timeout::TimeoutLayer` (30s) bounds the pre-response phase (query/domain lookups) and returns `408 Request Timeout`, and — because it is layered outside the concurrency permit — also bounds how long a request can queue for a permit before failing instead of queueing forever; `tower_http::timeout::ResponseBodyTimeoutLayer` (60s) applies a per-frame idle timeout to every response body, resetting each time the body is polled and yields a frame. The rate limiter (where present) is now threaded into `apply_common_layers` itself and layered between `security_middleware` and `GlobalConcurrencyLimitLayer`: inside CORS/security-headers/tracing, so a `429` still gets `X-Content-Type-Options`/`X-Frame-Options`/CSP headers, a correct CORS response instead of an opaque cross-origin network error, and still shows up in request traces — but outside the concurrency permit, so a `429` never consumes one. (An intermediate revision applied the rate limiter fully outermost of everything, which regressed those three properties for every `429`/`503`; that revision does not ship.) It previously sat inside the permit in both `create_pjs_router_with_rate_limit_and_config` and `create_pjs_router_with_rate_limit_and_auth`. **What is not fixed:** `ResponseBodyTimeoutLayer`'s idle timeout only resets when the body is *polled*; hyper stops polling a response body once its outbound buffer fills waiting on the client to read the socket, so a client that stops reading entirely — the scenario in the original #515 report — is not caught by this layer, and on the current `GET .../frames/stream` route (#511) specifically the layer is close to a no-op even for the producer-stall case it does cover, since that route's frames are fully materialized into an in-memory `Vec` before streaming starts. Closing that gap needs connection/socket-level accounting in whatever calls `axum::serve`, which `pjs-core` does not currently expose a public API for. Adds the `timeout` feature to the `tower-http` dependency and the `limit` feature to `tower` (#515)
- **Closes #515** (`infrastructure::http::serve`, new module) — closes the residual gap the partial mitigation above explicitly left open: a client that stops reading the response socket entirely was never caught by `ResponseBodyTimeoutLayer`, since that layer's idle timeout only resets when the body is *polled*, and hyper stops polling once its outbound buffer fills waiting on the client. A new `serve_with_limits`/`ConnectionLimits` pair (`http-server`-gated) replaces `axum::serve` at the process's actual `TcpListener` accept loop — the layer `pjs-core` previously had no access to, since it only builds `Router`s — with a `hyper_util::server::conn::auto::Builder`-based accept loop that: bounds each connection's total lifetime with a `tokio::time::timeout` wrapping the per-connection serve future (default 300s; this catches a stalled write regardless of whether hyper is parked mid-`poll_flush`, closing the exact blind spot `ResponseBodyTimeoutLayer` has); bounds request-header read time via `header_read_timeout` (default 10s, ~6x tighter than hyper's own 30s default) — mitigating a separate, previously-unaddressed slowloris-on-headers gap found during this work, since `axum::serve` never configures a hyper timer and so silently disables `h1_header_read_timeout` entirely (calling `header_read_timeout` without first calling `.timer(...)` on the builder panics on the first connection, so both are wired together); and caps total concurrent connections via an `Arc<Semaphore>` acquired before `accept()` (default 1024), a distinct axis from `GlobalConcurrencyLimitLayer`'s handler-concurrency cap above. `ConnectInfo<SocketAddr>` is preserved per connection, since this crate's own WebSocket upgrade handler and per-IP rate limiter both require it. A long-lived route such as this crate's own `/pjs/ws/{session_id}` WebSocket upgrade must set `max_connection_duration: None` to avoid being killed by the deadline; `pjs-demo`'s three server binaries now use `serve_with_limits` in place of `axum::serve`, with `websocket_streaming.rs` opting out of the deadline accordingly (#523)
- **BREAKING** — **Closes #525, mitigates #526** (`infrastructure::http::serve`, `security::rate_limit`) — hardens `serve_with_limits` against three gaps a follow-up audit of #523 found: (1) hyper-util's `auto::Builder` sniffs the h1/h2 preface (`ReadVersion`) before any protocol builder engages and has no timer of its own, so a connection that sends zero bytes was previously bounded only by `max_connection_duration` (300s) instead of the much tighter `header_read_timeout` (10s default) — `serve_with_limits` now gates `TokioIo::new(stream)` behind a `tokio::time::timeout`-wrapped `stream.readable()` wait, which is readiness-only and consumes no bytes, closing the zero-byte case for h1 and h2 alike (a client that sends exactly one byte then stalls still rides out the full `max_connection_duration`, a residual documented on the gate itself); (2) hyper's own h2 default of `max_concurrent_streams` (200, explicitly documented upstream as unstable) is now tightened via two new `ConnectionLimits` fields, `max_concurrent_streams` (default `Some(128)`) and `max_connections_per_ip` (default `Some(64)`, `None` to disable) — the latter enforced by a new private `security::rate_limit::WebSocketRateLimiter` instance owned by `serve_with_limits` (never sharing state with any `RateLimitMiddleware` the router applies, keyed on a peer IP masked to its IPv6 /64 with IPv4-mapped and loopback addresses normalized before masking so distinct IPv4 clients behind a dual-stack listener don't collapse onto one shared budget) that rejects a connection before any task is spawned, releasing its `max_connections` semaphore permit immediately rather than after the fact; (3) `security::rate_limit::RateLimitGuard` no longer derives `Clone` — it implements `Drop` to decrement a connection counter, so a clone would have silently over-decremented it once a caller other than the sole existing one (which already wraps it in `Arc`) started cloning it directly

### Performance

- **BREAKING** `infrastructure::http::streaming`'s `format_frame_owned`/`format_batch_owned` now serialize each frame's `serde_json::Value` via `sonic_rs::to_vec` instead of `serde_json::to_vec` (`StreamFormat::Json`/`NdJson`/`ServerSentEvents`/`Binary`, all 5 call sites). An ad-hoc, not-committed criterion micro-benchmark against the exact frame shape these functions produce showed `sonic_rs::to_vec` consistently faster than `serde_json::to_vec`: roughly 1.6x-6.7x on single-frame payloads (0 B-16 KiB) and a smaller ~1.07x-4x on 100-frame batches (the shape `format_batch_owned` actually runs in production), varying with payload size and measurement noise. `StreamTransportError::Serialization` now wraps `sonic_rs::Error` instead of `serde_json::Error`, since `sonic_rs::to_vec` produces a different error type. **Breaking:** `StreamTransportError::Serialization`'s wrapped error type changed from `serde_json::Error` to `sonic_rs::Error`; any external code matching on this variant or constructing it directly must update accordingly (#508)

### Added

- Regression tests now assert `pjson_rs_domain::MAX_DESERIALIZE_DEPTH` (the `pjs-domain` deserialization recursion guard, a hard ceiling) stays equal to every other in-tree default that independently hardcodes the same depth-64 value — `pjs-core`'s `SecurityConfig::default().json.max_depth` and `SecurityConfig::development().json.max_depth`, `JiterConfig::default().max_depth` (`crates/pjs-core/src/config/security.rs`), and `pjs-wasm`'s `DEFAULT_MAX_DEPTH` (`crates/pjs-wasm/src/security.rs`) — since the constants cannot be shared directly (dependents depend on `pjs-domain`, not the reverse) and previously had no automated check keeping them in sync. `SecurityConfig::high_throughput()`/`low_memory()` intentionally diverge from this default and are not covered (#473)
- **BREAKING** `StreamRepositoryGat` gained five new required methods (`create_stream_atomic`, `start_stream_atomic`, `complete_stream_atomic`, `create_stream_patch_frames_atomic`, `close_session_atomic`) and `SessionStats` gained a new public `recent_avg_duration_ms: f64` field, as part of the #457/#458 concurrency fix described under Fixed. **Breaking:** any external `StreamRepositoryGat` implementor must add the five new methods, and any code constructing `SessionStats` via a struct literal must add the new field; pre-1.0 policy permits this without a deprecation cycle (#457, #458)
...
- New route `GET /pjs/sessions/{session_id}/streams/{stream_id}/frames/stream`, wiring `infrastructure::http::streaming` into a live handler for the first time — previously every type in that module (`StreamFormat`, `BatchFrameStream`, `create_streaming_response`/`_with_content_type`) was reachable only from this crate's own tests. Content is negotiated from the `Accept` header only (`text/event-stream` → SSE, `application/x-ndjson` → NDJSON, anything else including an absent header, `*/*`, or `application/octet-stream` → NDJSON-of-objects promoted to `application/x-ndjson`); the page is served via `BatchFrameStream` one frame per chunk, and the response carries `X-Total-Count` mirroring `FramesResponse::total_count`. This is a page-bounded batch stream, not a persistent SSE session — a browser `EventSource` must call `.close()` after the final event, since the route emits no `id:`/`retry:` sentinel; continuous/live push is still the existing WebSocket endpoint's job (#511)
- `pjs-bench`'s new `http_streaming` criterion benchmark (`crates/pjs-bench/benches/http_streaming.rs`), committed as a follow-up to the ad-hoc, uncommitted micro-benchmark #510 originally validated against. Two `sonic_rs`-vs-`serde_json` comparison groups (`http_streaming_serialization_many_small_calls`, `http_streaming_serialization_one_big_call`) isolate the exact primitive #510 swapped, at both call shapes `format_batch_owned` exercises (`infrastructure::http::streaming.rs`: one serializer call per frame for `Json`/`NdJson`/`ServerSentEvents`, one call over the whole batch for `Binary`, the latter not itself reachable from the live route since it normalizes `Binary` to `Json`) — measured ratio is sonic_rs ~1.4x-1.6x faster for many-small-calls (the route-reachable shape) and ~1.7x-1.8x for one-big-call across `frame_count` in `[10, 100, 1000]`, directionally consistent with #510's claim but well below its originally-claimed 1.6x-6.7x/1.07x-4x upper range. A third group, `http_streaming_batch_frame_stream_e2e`, benchmarks the actual production call chain (`BatchFrameStream::into_stream()`, drained via `futures::StreamExt::collect` inside a current-thread tokio runtime, bypassing `axum`/`hyper` framing) as a regression baseline — its `batch_size` axis (`[1, 100]`, `1` matching the `/frames/stream` route added in #511) reflects stream-yield/allocation amortization only, since `Json`/`ServerSentEvents` call the serializer once per frame regardless of `batch_size`. The answer to #514's "verify or correct #510's claim against the code path that now actually serves production traffic" question: serialization is only ~18-19% of the e2e route's per-request time (the rest is `frame_to_value`'s unchanged `serde_json`-based prep plus stream/async overhead), so substituting the `serde_json` arm's cost back into the e2e total puts sonic_rs's actual production-route contribution at roughly **9-11%**, not #510's originally-claimed multiple-x figure — that figure was, and remains, accurate only for the isolated serializer primitive, not for the route it now runs inside. All percentages are from one reference run on a shared, variably-loaded machine and are order-of-magnitude, not exact — see the bench's module doc for the stability caveat (#514)

### Fixed

- `SessionCommandHandler::save_and_publish` (`crates/pjs-core/src/application/handlers/command_handlers.rs`) persisted a clone of the mutated `StreamSession` to the repository *before* calling `session.take_events()`, so the stored session's `pending_events` buffer never got cleared — every subsequent `save_and_publish` call for that session republished the entire historical event backlog instead of just the events produced by that call, and `pending_events` grew unboundedly across the session's lifetime. `take_events()` is now called before the session is cloned and persisted, so the stored copy always has an empty `pending_events` buffer; at the time, the same reorder fixed all seven command handlers that routed through `save_and_publish` (`CreateSessionCommand`, `CreateStreamCommand`, `StartStreamCommand`, `GenerateFramesCommand`, `BatchGenerateFramesCommand`, `CompleteStreamCommand`, `CloseSessionCommand`) — `CreateSessionCommand::handle` previously had its own duplicated inline save/take_events/publish block with the same ordering bug, now replaced with a call to `save_and_publish` (#466). Five of those seven (`CreateStreamCommand`, `StartStreamCommand`, `CompleteStreamCommand`, `GenerateFramesCommand`, `CloseSessionCommand`) moved off `save_and_publish` entirely in the #457/#458 fix below, which drains events as part of an atomic repository call instead — only `CreateSessionCommand` and the unrouted `BatchGenerateFramesCommand` still rely on this reorder
...
- `pjs-demo`'s `websocket-streaming-server` `ClientCommand` parser (`servers::websocket_streaming`) declared `#[serde(deny_unknown_fields)]` on an internally-tagged enum with plain unit variants (`Pause`, `Resume`) — serde's derive only enforces `deny_unknown_fields` for struct variants, so the attribute was silently a no-op and `{"command":"pause","extra":true}` deserialized successfully as `Pause` instead of being rejected as a malformed command. Both variants now use empty-brace struct syntax (`Pause {}`, `Resume {}`), which makes `deny_unknown_fields` take effect; a payload with unexpected fields now hits the existing malformed-command `warn!` path, same as an unrecognized `command` value already did; as a side effect, a bare JSON array like `["pause"]` — previously accepted as `Pause` since serde read array element 0 as the tag — is now rejected too (#428)
...
- `pjs-core`'s `gat_performance_showcase` example no longer compares an unrelated no-op stub against real work, or prints unverified/false zero-allocation claims — `benchmark_response_creation` (which compared `create_streaming_response`'s literal-string stub against `create_health_response`'s real HashMap/String allocation) is removed, `benchmark_memory_allocation`'s "GAT with pooled objects" label and "Zero heap allocations for pooled responses" claim (there is no pooling on that path, and both `data.clone()` and the delegated `to_string()` allocate) are replaced with accurate wording, and several comments/println claims conflating a stack-allocated GAT `Future` type with a zero-allocation function body are reworded to describe only what's actually true (no `Box<dyn Future>` allocation) (#435). (This example is subsequently removed entirely later in this same release, alongside `StreamingAdapter`/`UniversalAdapter` — see `### Removed` below / #487.)
- `pjs-core`'s `object_pool_performance` example's `benchmark_vec_allocation`/`benchmark_hashmap_allocation` loops no longer get dead-code-eliminated in release builds (both the standard and pooled allocation loops now pass their result through `std::hint::black_box`), and the printed "x faster" result now reflects the actual measured direction instead of always claiming the pooled path is faster even when it measured slower (#429). (This example is subsequently removed entirely later in this same release, alongside the `object_pool` module it exercised — see `### Removed` below / #503.)
- `StreamSession::complete_stream` (`domain::aggregates::stream_session`) updated `stats.average_stream_duration_ms` with `(average_stream_duration_ms + duration_ms) / 2.0` — a fixed-alpha-0.5 exponential moving average, not a true running arithmetic mean, so the field's value depended on stream completion order rather than converging to the actual average duration. It now uses Welford's incremental-mean formula (`average_stream_duration_ms += (duration_ms - average_stream_duration_ms) / completed_streams as f64`), matching the field's documented meaning (#453)
- Every write command handler in `SessionCommandHandler` (`application::handlers::command_handlers`) — `CreateStreamCommand`, `StartStreamCommand`, `CompleteStreamCommand`, `GenerateFramesCommand`, `CloseSessionCommand` — read a `StreamSession` out of the repository, mutated the clone, then wrote it back with no compare-and-swap or per-session lock around the read-modify-write cycle: under concurrent commands on the *same* session (e.g. one client's `CompleteStreamCommand` racing another's `StartStreamCommand`), the later writer's `save_session` could silently overwrite the earlier writer's update, permanently under-counting `stats.completed_streams` and biasing every subsequent sample of `average_stream_duration_ms`'s Welford running mean, since `completed_streams` is that formula's divisor and, unlike the pre-#453 EMA, never decays; a lost `StreamCompleted` mutation could also resurrect a completed stream as `Active`, letting it be completed a second time (#457). `StreamRepositoryGat` gained five new atomic methods — `create_stream_atomic`, `start_stream_atomic`, `complete_stream_atomic`, `create_stream_patch_frames_atomic`, `close_session_atomic` — each applying its `StreamSession` mutation and draining the session's pending events while holding the underlying `DashMap`'s per-key shard lock (`InMemoryStore::update_with`), so no two commands touching the same session can interleave their read-modify-write; `StreamSession` gained a matching `create_stream_with_config` method combining stream creation and optional per-stream config into one atomic domain operation. All five command handlers above now call their atomic repository method directly instead of the generic load + mutate + `save_session` cycle (`CreateSessionCommand` is unaffected — it always targets a brand-new session id nothing else can yet reference, so it has no read-modify-write to race; `BatchGenerateFramesCommand` was left on the old pattern at the time — see the #477 entry below for its subsequent fix). `create_stream_patch_frames_atomic`'s patch extraction — a traversal proportional to the stream's full source payload, not to `max_frames` — runs on a snapshot taken *before* acquiring the lock rather than inside it, so it no longer blocks unrelated sessions hashing to the same `DashMap` shard for the traversal's duration; only the cheap, bounded commit step holds the lock, re-checking the stream is still `Streaming` in case a concurrent call finished it first. `GenerateFramesCommand`'s handler now publishes the drained events immediately after the atomic call, before the fallible `append_frames` step, so a persistence failure can no longer discard already-drained events. Also fixes #458: `SessionStats` gained a `recent_avg_duration_ms` field — a fixed-alpha-0.5 exponential moving average over completed-stream durations, seeded with the first observed duration — giving a recency-weighted companion to `average_stream_duration_ms`'s lifetime mean, which is unchanged from #453 (this is not a revert). `SessionHealthSnapshot::metrics` now reports this value under a `recent_avg_duration_ms` key instead of `avg_duration_ms` (which read `average_stream_duration_ms`, the lifetime mean, before this change — the EMA did not exist prior to this PR), so the health/monitoring path's recency-sensitive signal is named consistently with the `SessionStats` field it mirrors and with `GET /pjs/sessions/{id}/stats`'s response, while the lifetime field itself stays a correct running average; all fields update together inside the same atomic mutation, so the concurrency fix above covers them jointly with no separate race window (#457, #458)
- **BREAKING** `BatchGenerateFramesCommand` was the one command handler left out of the #457 atomicity fix above — it still routed through `find_session` + `StreamSession::create_priority_frames` + `save_session`, so a concurrent `*_atomic` call on the same session landing between its read and its blind-overwrite `save_session` could silently lose that call's `SessionStats`/event update, same as #457's original bug. `StreamRepositoryGat` gained a sixth atomic method, `batch_generate_frames_atomic`; `SessionCommandHandler::handle::<BatchGenerateFramesCommand>` now calls it directly and publishes the drained events immediately after, before the fallible frame-store persistence step, matching `GenerateFramesCommand`'s existing ordering. `StreamSession` gained `extract_prioritized_patches_for_active_streams`/`commit_priority_frames`, splitting `create_priority_frames`'s per-stream-payload traversal (proportional to stream count, not just payload size) from its commit step, the same extract-then-commit pattern `create_stream_patch_frames_atomic` already used — the production repository runs the traversal on a pre-lock snapshot and holds the `DashMap` shard lock only for the commit step, instead of holding it for every stream's full traversal; that commit step is itself proportional to the total patches extracted across every `Streaming` stream, not to `max_frames`/`batch_size`. `commit_priority_frames` also now skips (rather than erroring the whole batch on) a stream that is not `Streaming` — `create_priority_frames`'s previous `is_active()`-then-`?` loop could both abort the batch on a merely-`Preparing` stream and, once the atomic method mutated the stored session in place instead of a discardable clone, permanently persist earlier-processed streams' partial frame bookkeeping while the batch's own session-level stats/event never landed. The stale doc comment on `StreamRepositoryGat::save_session` naming `BatchGenerateFramesCommand` as the one exception is updated to match. **Breaking:** any external `StreamRepositoryGat` implementor must add `batch_generate_frames_atomic` (#477)
- `PartialParseResult::value` doc comment (`crates/pjs-core/src/parser/partial.rs`) incorrectly claimed that for input like `{"a":1,"b":[2,3` the returned value would be `{"a": 1}` with the trailing array dropped entirely. The actual behavior is that fully-parsed array elements are retained — `{"a": 1, "b": [2, 3]}` — and only the syntactically incomplete trailing token (the unclosed `]`) is dropped. Doc comment corrected, and a new `# Examples` doctest added demonstrating the correct behavior (#481)
- `CompressionBombProtector` and `CompressionBombDetector` module docs (`crates/pjs-core/src/security/compression_bomb.rs`) and `SecureCompressor` module docs (`crates/pjs-core/src/compression/secure.rs`) read as an unconditional security guarantee that decompression is always guarded. Clarified that while the protection is fully implemented and tested, `decompress_protected`/`decompress_nested` currently have no production callers in this codebase — the only live usage is outbound compression via `compress`, not inbound decompression. This avoids a false sense of active protection on a path that isn't yet wired up (#482)
- `partial` module docs (`crates/pjs-core/src/parser/partial.rs`) and the `Parser::parse_partial` doc comment (`crates/pjs-core/src/parser/mod.rs`) read as if the DoS-hardened partial/streaming JSON parser were already wired into production. Clarified that nothing in this workspace calls `Parser::parse_partial`/`PartialJsonParser::parse_partial` or constructs `JiterPartialParser` outside this module's own unit tests, that the `partial-parse` feature gating it is not part of `pjs-core`'s default feature set, and that it is maintained for a future streaming ingestion feature rather than being battle-tested against production traffic today (#485)
- `search_sessions`'s (`GET /pjs/sessions/search`) `sort_by` query parameter hand-rolled its own `match` on the raw string (`"created_at" => Some(...)`, ..., `_ => None`), duplicating exactly what `SessionSortField`'s existing `#[serde(rename_all = "snake_case")]` derive already specifies, and silently fell through to `None` (sorting skipped) on any unrecognized value instead of rejecting the request (#492). A new `parse_sort_field` helper now deserializes the raw value through `SessionSortField`'s serde derive, giving that derive a live caller (#494); an unrecognized or empty (`?sort_by=`) value now returns `400` with the API's standard `{"error": ...}` envelope, matching the existing `?state=` behavior. **Note:** this is a behavior change for existing clients sending an unrecognized or empty `sort_by` — a request that previously returned `200` with results silently unsorted now returns `400`. The sibling `sort_order` parameter had the same gap; see the #497 fix below, which brings it to the same strict validation
- `search_sessions`'s (`GET /pjs/sessions/search`) `sort_order` query parameter still hand-rolled a `match` on the raw string (`"asc"`/`"ascending"` => `Some(SortOrder::Ascending)`, ..., `_ => None`) and silently fell through to `None` (default sort order) on any unrecognized value, the same bug `sort_by` had before #492/#494/#496 — filed as #497. `application::queries::SortOrder` gained `#[serde(alias = "asc")]`/`#[serde(alias = "desc")]` on its variants (its `#[serde(rename_all = "snake_case")]` derive already covered `ascending`/`descending`), and a new `parse_sort_order` helper — mirroring `parse_sort_field`, delegating to that derive rather than hand-rolling — now rejects an unrecognized or empty (`?sort_order=`) value with `400` and the API's standard `{"error": ...}` envelope instead of silently ignoring it; the accepted spellings (`asc`, `ascending`, `desc`, `descending`, case-sensitive) are unchanged. **Note:** this is a behavior change for existing clients sending an unrecognized or empty `sort_order` — a request that previously returned `200` with the default sort order now returns `400`
- **BREAKING** `BatchGenerateFramesCommand::priority_threshold` was carried through the handler as a `PriorityDto` but never actually read: `batch_generate_frames_atomic` (`domain::ports::gat::StreamRepositoryGat`) hardcoded `Priority::BACKGROUND` internally, so every batch request effectively used the lowest possible threshold regardless of what the caller asked for. `StreamRepositoryGat::batch_generate_frames_atomic` and `StreamSession::create_priority_frames` now take a `priority_threshold: Priority` parameter and thread it through to `extract_prioritized_patches_for_active_streams`, exactly mirroring `create_stream_patch_frames_atomic`'s existing single-stream parameter. **Breaking:** any external `StreamRepositoryGat` implementor must add the new parameter to `batch_generate_frames_atomic` (#498)
- `Stream::commit_patch_frames`, `StreamSession::commit_patch_frames_for_stream`, and `GatInMemoryStreamRepository::create_stream_patch_frames_atomic`'s doc comments each independently mischaracterized the patch-to-frame commit step as "the cheap, `O(max_frames)` half" of frame generation (pre-existing since #472); `Stream::extract_prioritized_patches`'s doc made the same claim indirectly, describing its sibling as "the cheap commit step". `max_frames` only bounds the number of frames *returned* — `batch_patches_into_frames` still clones every extracted patch while chunking it into frames, so the actual cost is `O(patches.len())`, proportional to the total number of patches extracted (which scales with source payload size), not to `max_frames`. All four doc comments are corrected to describe this accurately; the sibling multi-stream path (`StreamSession::commit_priority_frames`, `batch_generate_frames_atomic`, and the `gat_memory_repository` module doc) already documented this correctly and is unchanged. `StreamSession::extract_prioritized_patches_for_active_streams` and `commit_priority_frames` also silently discarded *any* error from a `Streaming` stream (via `.ok()` / `let Ok(..) else { continue }`) as if the only possible cause were the already-filtered non-`Streaming` state, ignoring `Stream::extract_prioritized_patches`'s other error source — a `Streaming` stream with no `source_data`. No current in-crate code path can actually produce that combination (`source_data` is set once in `Stream::new` and never reassigned), so this is defensive/future-proofing rather than a live gap; both call sites now log the discarded error via `tracing::debug!` before skipping the stream regardless (#500)
- `StreamSession::commit_priority_frames` (`domain::aggregates::stream_session`) built each active stream's candidate frames via `Stream::commit_patch_frames`, which unconditionally committed `next_sequence`/`stats` bookkeeping for every frame it built — before `commit_priority_frames`'s own cross-stream priority sort and `.take(batch_size)` truncation decided which frames actually survived. Frames dropped by that truncation had already consumed a sequence number and inflated their child `Stream`'s own `stats`, permanently desyncing per-stream stats/sequence numbering from the frames the caller actually received. `Stream` bookkeeping is now split into a pure `chunk_patches_for_commit` (chunks patches into per-frame groups, no mutation) and a mutating `finalize_patch_frame` (assigns the sequence number, builds the `Frame`, commits stats) — `commit_patch_frames`'s existing always-commit-everything behavior (used by the single-stream `create_stream_patch_frames`/`create_stream_patch_frames_atomic` path) is unchanged, just composed from these two steps. `commit_priority_frames` now chunks every stream's patches first, decides the surviving subset by priority without mutating anything, then finalizes only the survivors — walking candidates in each stream's original chunk order so `next_sequence` still advances monotonically in source-traversal order rather than cross-stream priority order — before re-sorting the finalized frames by priority to preserve the existing external ordering contract. Incidentally also fixes duplicate sequence numbers previously assigned when a single `commit_patch_frames` call produced multiple frames for one stream (#506)
- **BREAKING** `StreamFormat::from_accept_header` (`infrastructure::http::streaming`) ignored RFC 9110 §12.5.1 quality values entirely — it walked the raw header string through a fixed `contains` chain (`text/event-stream`, then `application/x-ndjson`, then `application/octet-stream`), so `Accept: application/json, text/event-stream;q=0.1` picked SSE, the client's *least* preferred type, and `contains` also substring-matched (`application/x-ndjson-plus` matched `NdJson`). This was previously unreachable in production — nothing in this crate's router called it — but becomes live, user-facing behavior with the new route above, so it is fixed in the same change rather than shipped broken. It now parses each comma-separated entry's `q` parameter (defaulting to `1.0`, `q=0` an explicit rejection, an unparsable or non-finite `q` — including `nan`/`inf`/`-inf`, which `f32::from_str` parses successfully and which would otherwise poison the max-comparison and win by default — also dropping that entry), compares media ranges by exact case-insensitive match, and returns the highest-`q` surviving entry (ties broken by first occurrence, not media-range specificity); `*/*` and `application/*` vote for the default (`StreamFormat::Json`) at their own `q` rather than being excluded from the race, so e.g. `text/event-stream;q=0.1, */*` correctly prefers the wildcard's implicit `q=1.0` over the low-`q` explicit type. At most 16 comma-separated entries are considered (bounded iteration over untrusted input); a missing header, an unparsable header value, or no entry surviving negotiation all still fall back to `StreamFormat::Json`, matching the previous default behavior. **Breaking:** any `Accept` header whose highest-preference entry was previously shadowed by an earlier, lower-`q` `contains` match now resolves to the client's actual preference instead; pre-1.0, no deprecation cycle (#511)
- `create_streaming_response` (`infrastructure::http::streaming`) manually set `Transfer-Encoding: chunked` on `StreamFormat::NdJson` responses and `Connection: keep-alive` on `StreamFormat::ServerSentEvents` responses. Application code must not set transfer-framing or connection-management headers by hand — the body encoder owns both decisions. This was latent (nothing called this response path in production before the route above), but concretely reachable now: on HTTP/2, both `Transfer-Encoding` and `Connection` are illegal headers and hyper logs a `WARN` on every response that carries either, and SSE — which set `Connection: keep-alive` — is a first-class negotiated format on the new route; on HTTP/1.1 a manually-set `Transfer-Encoding` can also collide with body-length-derived framing elsewhere in the stack. Neither header is set any more; framing and connection management are left entirely to the encoder. `X-Accel-Buffering` (a reverse-proxy hint, not a connection-management header) is unaffected and still set on SSE responses (#511)
- `GET .../frames/stream`'s per-frame JSON diverged from the buffered `GET .../frames` route: a hand-rolled `frame_to_value` helper (`infrastructure::http::streaming`) renamed the `frame_type` field to `type`, `Debug`-formatted the frame type instead of using its serde representation, and dropped `stream_id` entirely — even though `Frame`'s own derived `Serialize` (already used by the buffered route) produces the correct shape for free. `format_frame_owned`/`format_batch_owned` now serialize each `Frame` directly via `sonic_rs::to_vec(frame)` (`sonic_rs::to_vec(frames)` for the `Binary` batch variant), reusing `Frame`'s own `Serialize` impl instead of hand-rolling a `serde_json::Value`; this also avoids reintroducing the `serde_json::Value` intermediate #508 removed from this same output path, and keeps `StreamTransportError::Serialization` wrapping `sonic_rs::Error` end-to-end instead of adding a second, differently-typed error path for a `serde_json::to_value` call that could not practically fail (#516)
- `PriorityFrameStream`'s private `PriorityFrame` wrapper (`infrastructure::http::streaming`) hand-implemented `PartialEq`/`Ord` over a copied `priority: u8` field instead of delegating to `Frame::priority()`'s own `Priority`, which already derives `Ord` — a pure refactor with no behavior change on its own, since `Priority`'s `Ord` and the old `u8` comparison were bit-identical. While rewriting the comparison, `Ord` also gained a `sequence` tiebreaker (`priority`, then `Reverse(sequence)`), so frames of equal priority — previously left to `BinaryHeap`'s unspecified relative order — are now emitted in ascending-sequence (FIFO) order within a priority tier (#517)
- `pjs-js-client`'s `BaseFrame` type declared a `type: FrameType` field with lowercase values (`'skeleton'`/`'patch'`/`'complete'`), but `transport/http.ts` parses the REST streaming routes' NDJSON output directly as `Frame` — neither the field name nor the values matched what either HTTP route actually emits (a `frame_type` field with capitalized `"Skeleton"`/`"Patch"`/`"Complete"`/`"Error"` values, via `Frame`'s derived `Serialize`). `FrameType`'s values are now capitalized to match and gained an `Error` member; `BaseFrame`'s field is renamed `type` -> `frame_type` (propagated through `frame-processor.ts`, `client.ts`, `utils/index.ts`, and the WASM-native frame construction in `wasm-backend.ts`/`wasm-parser.ts`, which is unrelated to the REST wire format but shares the same `Frame` type). `BaseFrame` also gained optional `stream_id`/`sequence` fields, matching the REST routes' shape (both absent on WASM-native frames, which have no stream identity; WASM-derived frames now populate the real `sequence` field directly instead of stashing it under `metadata.sequence`). **Breaking:** any external consumer of `pjs-js-client` reading `frame.type` or comparing against the old lowercase `FrameType` values must update to `frame.frame_type` and the new capitalized values. `SkeletonFrame`/`PatchFrame`/`CompleteFrame` also modeled their frame-specific data as flattened top-level fields (`data`, `patches`, `checksum`) instead of nested under `payload`, matching neither `Frame::skeleton` (payload is the raw skeleton JSON, not a `data` wrapper), `Frame::patch` (payload is `{"patches": [...]}`, not a top-level `patches` array), nor `Frame::complete` (payload is `{"checksum": ...}` or `{}`, not a top-level `checksum`). `SkeletonFrame.data` is renamed `payload`; `PatchFrame.patches`/`CompleteFrame.checksum` move under a new `payload` field on each (`{ patches: [...] }` / `{ checksum?: ... }` respectively); `json-reconstructor.ts`'s `processSkeleton`/`applyPatch` and `client.ts`'s patch-frame handling read the new nested locations. **Breaking:** any external consumer constructing or reading `SkeletonFrame.data`, `PatchFrame.patches`, or `CompleteFrame.checksum` directly must update to the corresponding `payload`-nested field (#516)

### Changed

- The `StreamRepositoryGat` test mock used by unit tests in `application::handlers::command_handlers`, `application::handlers::query_handlers`, and `infrastructure::http::axum_adapter` was implemented three separate times (near-identical ~90-line boilerplate each, including all `*_atomic` methods from #457/#472). Consolidated into a single `pub(crate)` mock in a new `#[cfg(test)]`-only `crates/pjs-core/src/test_support.rs`, imported by all three test modules; test behavior is unchanged. The separate `StreamRepositoryGat` mocks in `tests/common/mod.rs` (a different compilation target), `domain::services::gat_orchestrator`, and `infrastructure::http::handlers::dictionary` were left as-is — out of scope for this consolidation — and only received the new `batch_generate_frames_atomic` method needed to keep implementing the trait (#478)
- `DomainEvent::stream_id()` (`crates/pjs-domain/src/events/mod.rs`) now matches every `DomainEvent` variant explicitly instead of falling back to a wildcard `_ => None` arm, matching the exhaustive style already used by its sibling accessors `timestamp()`, `session_id()`, and `event_type()` (and `occurred_at()`, fixed the same way in #434). A future variant carrying a `stream_id` field will now fail to compile against this match instead of silently returning `None`. No behavior change (#447)
- **BREAKING** `CreateStreamCommand::source_data` and `StartStreamRequest.data` (`infrastructure::http::axum_adapter`) are now `pjson_rs_domain::value_objects::JsonData` directly instead of `serde_json::Value` — the `create_stream` HTTP handler previously parsed the request body into `serde_json::Value` (via axum's `Json` extractor) and then walked it a second time into `JsonData` through `JsonDataDto::from(...).into()` (`command_handlers.rs`), reallocating every `String`/`Vec`/`HashMap` in the payload twice. `JsonData` now implements `serde::Deserialize` directly via a hand-written `Visitor` over `deserialize_any`, so axum's `Json<StartStreamRequest>` extractor builds `JsonData` in a single pass with no intermediate tree; measured 7-16% deserialize latency reduction on payloads from ~1.2 KB to ~253 KB (in-tree criterion benchmark, two independent runs). `JsonData` also gained a matching hand-written `Serialize` impl producing plain JSON output — e.g. `{"a":1}` instead of the previously-derived tagged-enum shape `{"Object":{"a":{"Integer":1}}}` — to keep serialization/deserialization symmetric, measured 42-49% faster than the tagged-enum shape it replaces. This fixes a real latent bug as a side effect: `pjs-js-client`'s WASM parser read `payload.patches` directly off a patch frame's JSON payload, which was silently always `undefined` under the old tagged shape (`{"Object":{"patches":{"Array":[...]}}}`); it now reads the real patch array. **Breaking:** any external code deserializing `JsonData`'s old tagged wire format (`{"Integer": 1}`, `{"Object": {...}}`, etc.) directly must be updated to expect plain JSON instead; the now-fully-unused `JsonDataDto` DTO (`application::dto::json_data_dto`) is removed (#449)
...
- `AdaptiveStreamController::stream_frames` (`infrastructure::websocket`) now reads the session's live `ClientMetrics::recommended_frame_delay()` before sending each frame, instead of a hardcoded 10ms sleep — a previously-unused method already existed for this purpose but was never wired up. Currently a functional no-op end-to-end for HTTP-created sessions, since `create_session` always builds a single-frame plan (#431)
...
- `pjson-rs-domain`'s `DomainEvent::occurred_at` (`events::mod`) duplicated `DomainEvent::timestamp`'s identical ~20-arm match over every event variant. `occurred_at` now delegates to `timestamp` instead of re-matching; behavior and public API are unchanged (#434)
- `SonicParser::parse` (`parser::sonic`) no longer validates input UTF-8 via a separate scalar `std::str::from_utf8` call before handing a `&str` to `sonic_rs::from_str`; it now calls `sonic_rs::from_slice` directly on the raw `&[u8]`, which performs its own UTF-8 validation using SIMD (`simdutf8`) instead of `std`'s scalar validator. The pre-parse nesting-depth bound (`pre_validate_json_bytes`, formerly `pre_validate_json_string`) now scans raw bytes for the ASCII delimiters `{`/`[`/`}`/`]` directly instead of decoding via `.chars()`, with the same depth limit and bound logic preserved exactly. Invalid UTF-8 input is still rejected as a parse error, but as `Error::InvalidJson` (sonic-rs' own error) instead of `Error::Other` — a semver-visible error-variant change. (Note: `SonicParser` is subsequently removed later in this same release — see `### Removed` below.) (#454)
- `Schema`'s (`pjson-rs-domain`) and `SchemaDefinitionDto`'s (`pjson-rs`) deserialization recursion-depth guards, added independently in #470 and #476, duplicated the identical thread-local-counter + RAII-drop-guard + bounds-checked-entry pattern verbatim, differing only in the counter's name and the rejection message's type-name prefix. Extracted into one shared primitive, `pjson_rs_domain::value_objects::enter_deserialize_depth` (`crates/pjs-domain/src/value_objects/depth_guard.rs`), parameterized by a `&'static LocalKey<Cell<usize>>` counter reference and a `type_name: &str` used in the rejection message, returning a generic `DepthGuard` RAII type. Each caller still declares its own `thread_local!` counter (nesting depth is tracked independently per recursive type) but delegates the entry/guard logic to the shared function; behavior, the reused `MAX_DESERIALIZE_DEPTH` limit, and both callers' distinct rejection messages ("Schema nesting depth exceeds maximum of N" / "SchemaDefinitionDto nesting depth exceeds maximum of N") are unchanged (#483)
- **BREAKING** `Pagination::sort_by` (`domain::ports::repositories`) is now `Option<SessionSortField>` instead of `Option<String>`. Previously, `SessionQueryHandler`'s `SearchSessionsQuery` handler (`application::handlers::query_handlers`) converted the already-validated `SessionSortField` enum (parsed from the HTTP query string in `infrastructure::http::handlers::sessions`) back into a raw string before constructing `Pagination`, which `Pagination::validate()` then re-checked against a runtime whitelist (`ALLOWED_SORT_FIELDS`) — redoing, at runtime and fallibly, exactly the exhaustiveness the enum already guaranteed at compile time. `SessionSortField` is now defined in the domain layer (`domain::ports::repositories`) as the canonical type — `Pagination` is a domain port type and must not depend on the application layer for it — and `application::queries::SessionSortField` is a re-export of the same type, so no conversion happens at the query-handler boundary. `Pagination::validate()` no longer performs the string whitelist check (unreachable now that an invalid sort field cannot be constructed), and `domain::config::limits::ALLOWED_SORT_FIELDS` is removed along with its re-exports. `GatInMemoryStreamRepository`'s field-comparison logic now matches on the enum instead of `&str`. No behavior change for HTTP clients (that layer already silently ignores an unrecognized `sort_by` rather than rejecting it, unchanged by this PR). For direct Rust API callers, an invalid `sort_by` is now a compile error instead of a runtime `validate()` error (#479)
- **BREAKING** `Pagination` (`domain::ports::repositories`) is now generic over its sort-field type (`Pagination<S>`), instead of being hardcoded to `SessionSortField` — the type previously coupled every pagination use case to the session bounded context's sort vocabulary, even for `FrameRepositoryGat::get_frames_by_stream`, which has no relationship to session sorting at all. Two type aliases replace direct use of the bare type: `SessionPagination = Pagination<SessionSortField>` for session queries, and `FramePagination = Pagination<FrameSortField>` (a new enum, `Sequence`/`Priority`, grounded in `Frame::sequence()`/`Frame::priority()`) for frame queries; `StreamRepositoryGat::find_sessions_by_criteria` and `FrameRepositoryGat::get_frames_by_stream` now take `SessionPagination`/`FramePagination` respectively. `Pagination<S>`'s `Default`, `validate()`, and `new_validated()` are unchanged in behavior and carry no bound on `S`. **Breaking:** every existing `Pagination` call site must migrate to `SessionPagination` (a one-token rename — all pre-existing usages were session-listing sites); `Pagination` itself is no longer directly constructible without specifying `S` (#491)
- `StreamFormat::from_accept_header` (`infrastructure::http::streaming`) no longer breaks ties between candidates at equal `q` by first occurrence in the header; it now delegates to the `headers-accept` crate's RFC 9110 §12.5.1-conformant `Accept::negotiate` for this route's supported media ranges, which ranks by `q` value and then by media-range specificity (exact match > `application/*` > `*/*`) — e.g. `Accept: application/*;q=1.0, application/x-ndjson;q=1.0` now resolves to `StreamFormat::NdJson` instead of `StreamFormat::Json`. Everything else about the pre-migration hand-rolled parser's observable behavior is preserved exactly, each deliberately re-derived (not simply assumed) against `origin/main`'s prior implementation during review: the `MAX_ACCEPT_ENTRIES` (16) bound on comma-separated entries and the permissive fallback to `StreamFormat::Json` on a missing/unparsable header or no surviving candidate; a finite but out-of-range `q` (e.g. `q=5`) is clamped to `[0.0, 1.0]` and the entry is kept, not dropped; non-finite (`nan`/`inf`/`-inf`) or otherwise unparsable `q` values are dropped during sanitization, before the header ever reaches `headers-accept`'s parser; wildcard matching is restricted to exactly `*/*` and `application/*` as before, not the broader RFC-general `type/*` matching `headers-accept` itself supports; and a single malformed entry (e.g. `garbage!!`) is dropped individually rather than discarding the rest of the header, since `headers-accept`'s own `Accept::from_str` fails the entire header on one bad entry. There is a second, narrower behavior change beyond the specificity tie-break: `q=0` on a concrete media type is still an explicit rejection of that type, but is no longer removed from the candidate pool before matching begins the way `origin/main` removed it — `headers-accept`'s `negotiate()` instead locks each candidate onto its single best-specificity matching `Accept` entry and only then checks that entry's `q`, so a `q=0` entry on an exact type now excludes *only* that concrete candidate, rather than being stripped from the header outright. When a broader wildcard entry is also present, other candidates that only match via the wildcard can still win where they previously lost to the wildcard's hardcoded vote for `StreamFormat::Json`: `Accept: application/json;q=0, */*` now resolves to `StreamFormat::ServerSentEvents` instead of `StreamFormat::Json` — arguably more RFC 9110-correct, since an explicit rejection of one specific type is not grounds to also exclude a separately-stated wildcard from the negotiation (#518)

### Removed

- **BREAKING** `pjson-rs`'s `parser::{JsonValue, LazyArray, LazyObject, FieldRange}` (`parser/value.rs`) and the `parser::JsonScanner` trait plus `parser::ScanResult`/`parser::StringLocation` (`parser/scanner.rs`) — a second, unused "lazy zero-copy JSON value" implementation shipped alongside the real one (`parser::zero_copy`, SIMD-integrated and actually used by every production parsing path: `sonic.rs`, `simple.rs`, `zero_copy.rs`, `partial.rs`). `JsonScanner` had zero implementors anywhere in the workspace, so its `scan`/`parse_numeric_array`/`find_strings`/`find_structural_chars`/`supports_simd` methods were unreachable. `LazyArray::from_scan`/`LazyObject::from_scan` took a `ScanResult` parameter that was ignored (`_scan_result`) and did manual scalar scanning despite doc comments claiming SIMD-precomputed boundaries — a false claim. Neither `JsonValue::parse_raw()` nor `LazyArray`/`LazyObject` was called from any production code, example, or integration test outside `value.rs`'s own unit tests. Both source files and the two integration test files that exercised only this dead code (`tests/parser_scanner_comprehensive.rs`, `tests/parser_value_comprehensive.rs`) are removed entirely. **Breaking:** `pjson_rs::parser::{JsonValue, LazyArray, LazyObject, FieldRange, JsonScanner, ScanResult, StringLocation}` no longer exist; pre-1.0 breaking change, no deprecation cycle (#463)
- **BREAKING** `DecompressionMetadata::priority_hints` (`stream::compression_integration`) — a `HashMap<u8, String>` field always written empty (`create_decompression_metadata` never populated it beyond `HashMap::new()`) and read nowhere; decompression strategy selection is fully driven by the `strategy: CompressionStrategy` enum, which a parallel per-priority hint map would duplicate rather than extend. **Breaking:** `DecompressionMetadata` struct literals must drop the `priority_hints` field (#448)
- **BREAKING** `pjson_rs::{Frame, FrameHeader, FrameFlags}` and the `pjson_rs::frame` module (`frame.rs`) — the `#[repr(C, align(64))]` wire-format struct, distinct from and constantly confused with the real domain entity `pjson_rs_domain::entities::Frame` (still available as `pjson_rs::DomainFrame`). It was constructed only by the removed `SonicParser::parse` and had no reader anywhere in the streaming pipeline; `FrameHeader::sequence` was hardcoded to `0` despite documenting itself as an ordering/dedup key. **Breaking:** `pjson_rs::{Frame, FrameHeader, FrameFlags}` no longer exist; pre-1.0 breaking change, no deprecation cycle (#486)
- **BREAKING** `pjson_rs::semantic` and `pjson_rs::{SemanticMeta, SemanticType}` (`semantic.rs`) — the type-detection subsystem existed only to populate the now-removed `Frame::semantics` field and had no other caller. **Breaking:** `pjson_rs::semantic`, `pjson_rs::SemanticMeta`, `pjson_rs::SemanticType` no longer exist; pre-1.0 breaking change, no deprecation cycle (#488)
- **BREAKING** `pjson_rs::{Parser, SimpleParser, SonicParser, ParseConfig, ParseStats}` and `pjson_rs::parser::{simple, sonic}` — no production caller; HTTP ingestion uses axum's `Json<T>` extractor and `pjs-wasm` parses via `serde_json` directly, so this hybrid sonic-rs/serde façade parsed into the now-removed dead `Frame` type for no consumer. `ZeroCopyParser`/`LazyParser` and the `partial-parse` parser (`parser::partial`) are unaffected and remain the supported parsing entry points. Depth-bomb protection on those surviving parsers is unaffected: `ZeroCopyParser` (`parser/zero_copy.rs`) already calls `SecurityValidator::validate_json_depth` once per nesting level as it recurses, independently of the guard this removal affects, with its own test coverage. The #456 guard, `SecurityValidator::validate_json_depth_bytes` (a whole-input byte scan, distinct from `validate_json_depth`'s incremental check), is retained as a public security primitive with its regression tests intact, but this removal deletes its only production callers, so it is not currently wired into any live ingestion path; the secondary depth assertions inside the removed `parser/mod.rs`/`parser/sonic.rs` were redundant copies of that same guard, not independent coverage. `crates/pjs-bench/benches/simple_throughput.rs` drops its `pjs_parser` benchmark group (`benchmark_pjs_parser` and the `pjs_parser` arm of `benchmark_comparison`), which measured only this removed parser; `serde_json_parsing`, `sonic_rs_parsing`, and `parsing_comparison` (now `serde_json` vs. `sonic_rs` only) are unaffected. `crates/pjs-core/tests/security_comprehensive.rs` drops seven tests that exercised only `SimpleParser`/`SonicParser` as smoke-test parsers with no unique assertion; its `SecurityValidator`/`DepthTracker`-based depth-attack tests are untouched. **Breaking:** `pjson_rs::{Parser, SimpleParser, SonicParser, ParseConfig, ParseStats}` and `pjson_rs::parser::{simple, sonic}` no longer exist; pre-1.0 breaking change, no deprecation cycle (#486, #488)
...
- **BREAKING** `pjson-rs`'s `JsonAdapter` (`infrastructure::adapters::json_adapter`) — a second, independent hand-rolled `JsonData <-> serde_json::Value` conversion that disagreed with `JsonData`'s own canonical `Serialize` impl (added in #455) on non-finite floats: `JsonAdapter::to_serde_value` mapped `NaN`/`Infinity`/`-Infinity` to the JSON number `0`, while `JsonData`'s `Serialize` impl (via `serialize_f64`) maps them to `null`, matching `serde_json`'s own convention. `JsonAdapter::to_serde_value`/`from_serde_value` had zero production call sites anywhere in `pjs-core` or `pjs-wasm` outside their own unit tests — `JsonData` already provides the same conversions via `serde_json::to_value(&json_data)`/`From<serde_json::Value>`/its `Deserialize` impl, so the NaN/Infinity policy is now single-sourced through `JsonData`'s own `Serialize` impl instead of two disagreeing implementations. (The replacement inbound path isn't a byte-for-byte match on every edge case either: `From<serde_json::Value>` falls back to `JsonData::Float(0.0)` for a `serde_json::Number` that is neither a valid `i64` nor `f64` — e.g. an integer above `i64::MAX` — where the removed `from_serde_value` fell back to `JsonData::Integer(0)`; both are equally lossy placeholders for a case `serde_json::Number` shouldn't produce in practice, not a behavior regression.) The entire `JsonAdapter` struct (including `merge`, `deep_clone`, `validate_pjs_structure`, `extract_leaf_paths`, `parse_json_string`, `to_json_string`, `to_json_string_pretty` — likewise uncalled outside their own tests) is removed rather than partially deduplicated. **Breaking:** `pjson_rs::infrastructure::JsonAdapter` and `pjson_rs::infrastructure::adapters::JsonAdapter` no longer exist; use `serde_json::to_value(&json_data)` / `JsonData::from(serde_json_value)` / `JsonData`'s own `Serialize`/`Deserialize` impls instead (#460)
...
- **BREAKING** `pjson-rs`'s `CompositeEventPublisher` and `EventPublisherVariant` (`infrastructure::adapters::event_publisher`) — a single-variant "composite" dispatch wrapper over `InMemoryEventPublisher`, single-variant and caller-less since #8; zero production constructors, and its only caller was the crate's own `#[cfg(test)]` `test_composite_publisher`, removed alongside it (its `InMemoryEventPublisher` clone-shared-state assertion is already covered by `tests/event_publisher_comprehensive.rs`'s `test_clone_shares_state`). #436 surfaced this as dead code by deleting the stale commented-out `HttpEventPublisher` placeholder that once sat next to it, but was not itself the cause — the composite never had a second live variant to dispatch to. `InMemoryEventPublisher` itself, and the `EventPublisherGat` port it implements, are unaffected — construct it directly instead of wrapping it in the composite. **Breaking:** `pjson_rs::infrastructure::{CompositeEventPublisher, EventPublisherVariant}` and `pjson_rs::infrastructure::adapters::{CompositeEventPublisher, EventPublisherVariant}` no longer exist (#439)
- **BREAKING** `pjson-rs`'s `StreamingAdapter`/`StreamingAdapterExt` traits and `streaming_helpers` (`infrastructure::integration::streaming_adapter`), and `UniversalAdapter`/`UniversalAdapterBuilder`/`AdapterConfig` (`infrastructure::integration::universal_adapter`) — a ~950-line "universal framework-integration layer" with no working production entry point. `UniversalAdapter` was the trait's sole concrete implementation anywhere in the workspace, and no production code path (`infrastructure::http::*`, `infrastructure::websocket::*`, `pjs-demo`) ever constructed or called a `StreamingAdapter`; the only other impl was `ModernGatAdapter` in the now also-removed `crates/pjs-core/examples/gat_performance_showcase.rs`, a benchmark example, not shipped code. Even if reached, `UniversalAdapter::to_response`/`convert_request` were hardcoded to always return `Err(IntegrationError::UnsupportedFramework(...))`, so every public entry point (`create_streaming_response`, `create_json_response`, `create_sse_response`) always failed regardless of input, after first doing real work (SIMD-accelerated serialization, pooled `HashMap`/`Vec` allocation) that was then thrown away. The `pooled_builders` submodule of `object_pool.rs` (`PooledResponseBuilder`, `PooledSSEBuilder`) existed solely to build `UniversalResponse`/`ResponseBody` and is removed with it. The rest of `object_pool.rs` (`ObjectPool`/`PooledObject` and the global pool helpers `get_cow_hashmap`/`get_byte_vec`/etc.) and all of `simd_acceleration.rs` other than the already-removed `SimdJsonProcessor` (#468) are kept as-is — but this removal deletes their last production caller: `streaming_helpers::default_sse_response` (inside the now-removed `streaming_adapter.rs`) was the only production code invoking `SimdStreamProcessor`/`SimdFrameSerializer`, and no `src/` code outside `object_pool.rs`/`simd_acceleration.rs` themselves calls into either module any more. They are kept because evaluating their removal is a separate decision outside this issue's scope, not because they retain production callers; a follow-up issue tracks that evaluation. **Breaking:** `pjson_rs::{StreamingAdapter, StreamingAdapterExt, UniversalAdapter, AdapterConfig, IntegrationError, IntegrationResult, ResponseBody, StreamingFormat, UniversalRequest, UniversalResponse}` and `pjson_rs::infrastructure::integration::{streaming_adapter, universal_adapter, UniversalAdapterBuilder, streaming_helpers}` no longer exist; pre-1.0 breaking change, no deprecation cycle (#487)
- **BREAKING** `pjson-rs`'s `SimdJsonProcessor` (`infrastructure::integration::simd_acceleration`) — a standalone `sonic_rs::from_slice`-based JSON validator/parser (`validate_json`, `extract_priority_field`, `validate_batch`) with zero production call sites anywhere in `pjs-core`, `pjs-wasm`, or `pjs-demo`; only its own unit tests and the crate's integration test suite referenced it. At the time, the real JSON validate/parse path was `parser::SonicParser`/`parser::SimpleParser` — both are subsequently removed later in this same release, see `### Removed` below. `SimdStreamProcessor` (also in `simd_acceleration.rs`) is a separate, unrelated type that was used on the live SSE path (`streaming_helpers::default_sse_response`) at the time and was likewise unaffected by this fix — despite sharing a file, the two SIMD types never called each other at runtime. (That SSE path, and `streaming_helpers::default_sse_response` itself, is subsequently removed later in this same release along with `StreamingAdapter`/`UniversalAdapter` — see `### Removed` below — after which `SimdStreamProcessor` has no production caller either.) The #456/#462 depth-bypass hardening previously applied to `SimdJsonProcessor::validate_json`/`extract_priority_field` (see the `### Security` entry above) goes away with the type; the guard it mirrored, `SecurityValidator::validate_json_depth_bytes`, is retained as a public primitive with its regression tests intact, but this release also removes its only production callers — the parsers that remain (`ZeroCopyParser`/`LazyParser`) guard nesting depth via `SecurityValidator::validate_json_depth` (`parser/zero_copy.rs`), a separate incremental per-level check, instead. **Breaking:** `pjson_rs::infrastructure::integration::simd_acceleration::SimdJsonProcessor` no longer exists; for JSON parsing, use `ZeroCopyParser`/`LazyParser`, the `partial-parse` parser, or `serde_json`/`sonic_rs` directly (#468)
- **BREAKING** `pjson-rs`'s entire `infrastructure::integration` module — `object_pool` (`ObjectPool`, `PooledObject`, `CleaningPooledObject`, `PoolStats`, `GlobalPoolStats`, and the global pool helpers `get_cow_hashmap`/`get_string_hashmap`/`get_byte_vec`/`get_string_vec`/`get_global_pool_stats`) and `simd_acceleration` (`SimdFrameSerializer`, `SerializationStats`, `SimdStreamBuffer`, `simd_acceleration::SimdConfig` (distinct from the crate's own `config::SimdConfig`, which is unaffected and remains a root export), `SimdStreamProcessor`). Their last production caller, `streaming_helpers::default_sse_response`, went away with the `StreamingAdapter`/`UniversalAdapter` layer earlier in this same release (#487, see above), which explicitly deferred this decision to a follow-up; after that removal no `src/` code in `pjs-core`, `pjs-wasm`, or `pjs-demo` referenced either module, only the crate's own tests and the `object_pool_performance` example (both removed here). `SimdStreamProcessor`'s `process_to_json`/`process_to_sse`/`process_to_ndjson` duplicated the live, tested SSE/NDJSON path in `infrastructure::http::streaming` (`PriorityFrameStream`/`BatchFrameStream`), which is unaffected and remains the supported way to serialize frames for streaming; `parser::buffer_pool::{AlignedBuffer, BufferPool}` — a separate type with its own unrelated `PoolStats` — remains the crate's real hot-path allocator and is likewise unaffected. Dropping the module also removes the now-unused `crossbeam` and `once_cell` dependencies from `pjs-core`. **Breaking:** `pjson_rs::infrastructure::integration` and everything under it no longer exists; pre-1.0 breaking change, no deprecation cycle (#503)
- **BREAKING** `infrastructure::http::streaming`'s `AdaptiveFrameStream` and `PriorityFrameStream` — reachable only from this crate's own test suite (`tests/http_streaming_comprehensive.rs`), never wired into a production route; the #511 route (`GET .../frames/stream`) uses `BatchFrameStream` instead. `AdaptiveFrameStream` was rejected for live use during #511's design (`.local/plans/511-streaming-wireup-plan.md`) because its `StreamFormat::Json` arm emits separator-less concatenated JSON objects under an `application/json` label — a wire-format bug; `PriorityFrameStream` was rejected because the domain layer already sorts frames by descending priority before sequence assignment (`pjson_rs_domain::entities::stream::Stream`), making a second priority-reordering pass redundant. Their only-used-by-them helpers (`format_frame_owned`, `maybe_compress`) and dedicated unit/integration tests are removed alongside them; `BatchFrameStream`, `StreamFormat`, `StreamTransportError`, and both `create_streaming_response*` helpers are unaffected. **Breaking:** `pjson_rs::infrastructure::http::{AdaptiveFrameStream, PriorityFrameStream}` and the same two names under `infrastructure::http::streaming` no longer exist; pre-1.0 breaking change, no deprecation cycle (#513)
...
[Unreleased]: https://github.com/bug-ops/pjs/compare/v0.7.0...HEAD
[0.7.0]: https://github.com/bug-ops/pjs/compare/v0.6.3...v0.7.0
