...
use pjson_rs::{JsonReconstructor, PriorityStreamer, StreamFrame};
use rand::Rng;
...
async fn benchmark_pjs_streaming_with_network(
    ...
    while let Some(frame) = plan.next_frame() {
        ...
        let frame_size = match &frame {
            StreamFrame::Skeleton { data, .. } => {
                serde_json::to_string(data).unwrap_or_default().len() + 50
            }
            StreamFrame::Patch { patches, .. } => {
                patches.len() * 150 + 100 // Approximate patch overhead
            }
            StreamFrame::Complete { .. } => 50,
        };
