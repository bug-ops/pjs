...
use crate::domain::{
    ...
    entities::{Stream, Frame, stream::StreamConfig},
    events::DomainEvent,
};
...
impl StreamSession {
    ...
    pub fn activate(&mut self) -> DomainResult<()> {
        match self.state {
            ...
            _ => Err(DomainError::InvalidStateTransition(
                format!("Cannot activate session from state: {:?}", self.state)
            )),
        }
    }
    ...
    pub fn create_stream(&mut self, source_data: JsonValue) -> DomainResult<StreamId> {
        ...
        if self.streams.len() >= self.config.max_concurrent_streams {
            return Err(DomainError::TooManyStreams(
                format!("Maximum {} concurrent streams exceeded", self.config.max_concurrent_streams)
            ));
        }
        
        let stream = Stream::new(self.id, source_data, self.config.default_stream_config.clone());
    ...
    pub fn close(&mut self) -> DomainResult<()> {
        match self.state {
            SessionState::Active => {
                ...
                let active_stream_ids: Vec<_> = self.streams
                    .iter()
            ...
            _ => Err(DomainError::InvalidStateTransition(
                format!("Cannot close session from state: {:?}", self.state)
            )),
        }
    }
    ...
    pub fn health_check(&self) -> SessionHealth {
        ...
        let failed_count = self.streams.values().filter(|s| matches!(s.state(), crate::domain::entities::stream::StreamState::Failed)).count();
