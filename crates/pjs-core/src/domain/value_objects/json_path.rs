...
impl JsonPath {
    ...
    pub fn parent(&self) -> Option<Self> {
        ...
        if let Some(pos) = self.0.rfind('.') {
            if pos > 1 { // Ensure we don't return empty path
    ...
    fn validate(path: &str) -> DomainResult<()> {
        ...
        if !path.starts_with('$') {
            return Err(DomainError::InvalidPath("Path must start with '$'".to_string()));
        }
            ...
            match ch {
                '.' => {
                    ...
                    if !key.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-') {
                        return Err(DomainError::InvalidPath(
                            format!("Invalid characters in key '{}'", key)
                        ));
                    }
                },
                '[' => {
                ...
                _ => {
                    return Err(DomainError::InvalidPath(
                        format!("Unexpected character '{}' in path", ch)
                    ));
                }
            }
        }
...
mod tests {
    ...
    fn test_path_operations() {
        ...
        let path = root.append_key("users").unwrap()
                      .append_index(0)
                      .append_key("name").unwrap();
    ...
    fn test_last_segment() {
        ...
        assert_eq!(path1.last_segment(), Some(PathSegment::Key("users".to_string())));
