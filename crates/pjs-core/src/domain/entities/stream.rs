...
use crate::domain::{
    ...
    value_objects::{Priority, JsonPath, StreamId, SessionId},
    entities::Frame,
};
...
impl Stream {
    ...
    pub fn start_streaming(&mut self) -> DomainResult<()> {
        match self.state {
            ...
            _ => Err(DomainError::InvalidStateTransition(
                format!("Cannot start streaming from state: {:?}", self.state)
            )),
        }
    }
    ...
    pub fn complete(&mut self) -> DomainResult<()> {
        match self.state {
            ...
            _ => Err(DomainError::InvalidStateTransition(
                format!("Cannot complete stream from state: {:?}", self.state)
            )),
        }
    }
    ...
    pub fn fail(&mut self, error: String) -> DomainResult<()> {
        match self.state {
            ...
            _ => Err(DomainError::InvalidStateTransition(
                format!("Cannot fail stream from state: {:?}", self.state)
            )),
        }
    }
    ...
    pub fn cancel(&mut self) -> DomainResult<()> {
        match self.state {
            ...
            _ => Err(DomainError::InvalidStateTransition(
                format!("Cannot cancel stream from state: {:?}", self.state)
            )),
        }
    }
    ...
    pub fn create_skeleton_frame(&mut self) -> DomainResult<Frame> {
        ...
        let skeleton_data = self.source_data
            .as_ref()
            .ok_or_else(|| DomainError::InvalidStreamState(
                "No source data available for skeleton".to_string()
            ))?;
    ...
    pub fn create_patch_frames(
        ...
        let source_data = self.source_data
            .as_ref()
            .ok_or_else(|| DomainError::InvalidStreamState(
                "No source data available for patches".to_string()
            ))?;
    ...
    pub fn is_finished(&self) -> bool {
        matches!(self.state, StreamState::Completed | StreamState::Failed | StreamState::Cancelled)
    }
    ...
    fn record_frame_created(&mut self, frame: &Frame) {
        ...
        self.stats.average_frame_size = self.stats.total_bytes as f64 / self.stats.total_frames as f64;
        ...
        match data {
            JsonValue::Object(obj) => {
                ...
                for (key, value) in obj.iter() {
                    skeleton.insert(key.clone(), match value {
...
mod tests {
    ...
    fn test_frame_creation() {
        ...
        assert_eq!(skeleton.frame_type(), &crate::domain::entities::frame::FrameType::Skeleton);
