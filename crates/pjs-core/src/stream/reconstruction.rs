...
use crate::Result;
use crate::stream::{StreamFrame, JsonPath, JsonPatch, PatchOperation, PathSegment};
...
impl JsonReconstructor {
    ...
    fn process_frame(&mut self, frame: StreamFrame) -> Result<ProcessResult> {
        ...
        match frame {
            StreamFrame::Skeleton { data, priority: _, complete: _ } => {
    ...
    fn set_at_path(&mut self, path: &JsonPath, value: JsonValue) -> Result<()> {
        ...
        match parent {
            ...
            _ => {
                return Err(crate::Error::Other("Cannot set field on non-object/array".to_string()));
            }
        }
    ...
    fn append_at_path(&mut self, path: &JsonPath, values: Vec<JsonValue>) -> Result<()> {
        ...
        match target {
            ...
            _ => {
                return Err(crate::Error::Other("Cannot append to non-array".to_string()));
            }
        }
    ...
    fn remove_at_path(&mut self, path: &JsonPath) -> Result<()> {
        ...
        match parent {
            ...
            _ => {
                return Err(crate::Error::Other("Cannot remove from non-object/array".to_string()));
            }
        }
    ...
    fn get_parent_mut(&mut self, path: &JsonPath) -> Result<&mut JsonValue> {
        ...
        for segment in parent_segments.iter().skip(1) { // Skip root
            match segment {
                PathSegment::Key(key) => {
                ...
                PathSegment::Index(idx) => {
                    if let JsonValue::Array(arr) = current {
                        current = arr.get_mut(*idx)
                            .ok_or_else(|| crate::Error::Other(format!("Index out of bounds: {}", idx)))?;
                    } else {
    ...
    fn get_mut_at_path(&mut self, path: &JsonPath) -> Result<&mut JsonValue> {
        ...
        for segment in path.segments().iter().skip(1) { // Skip root
            match segment {
                PathSegment::Key(key) => {
                    if let JsonValue::Object(map) = current {
                        current = map.get_mut(key)
    ...
    pub fn progress(&self) -> f32 {
        if self.is_complete {
            ...
            (self.stats.frames_processed as f32 / (self.stats.frames_processed + self.frame_queue.len() as u32) as f32)
...
mod tests {
    ...
    fn test_progress_calculation() {
        ...
        assert!(progress > 0.0 && progress < 1.0, "Progress was: {}", progress);
