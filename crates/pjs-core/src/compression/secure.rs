...
impl SecureCompressor {
    ...
    pub fn decompress_protected(&self, compressed: &SecureCompressedData) -> Result<Vec<u8>> {
        ...
        match protector.read_to_end(&mut decompressed) {
            Ok(_) => {
                ...
                // Final validation
                self.detector.validate_result(compressed.data.len(), decompressed.len())?;
                
                Ok(decompressed)
            },
            Err(e) => {
                warn!("Decompression failed with protection: {}", e);
                Err(Error::SecurityError(format!("Protected decompression failed: {}", e)))
            }
        }
    }
    ...
    pub fn decompress_nested(&self, compressed: &SecureCompressedData, depth: usize) -> Result<Vec<u8>> {
        ...
        let cursor = Cursor::new(&compressed.data);
        let mut protector = self.detector.protect_nested_reader(cursor, compressed.data.len(), depth)?;
        ...
        match protector.read_to_end(&mut decompressed) {
            Ok(_) => {
                ...
                if stats.compression_depth > 0 {
                    warn!("Nested decompression detected at depth {}", stats.compression_depth);
                }
            ...
            Err(e) => {
                ...
                Err(Error::SecurityError(format!("Nested decompression failed: {}", e)))
            }
        }
    }
...
impl SecureDecompressionContext {
    ...
    pub fn start_stream(&mut self, compressed_size: usize) -> Result<CompressionBombProtector<Cursor<Vec<u8>>>> {
        ...
        let protector = self.detector.protect_nested_reader(cursor, compressed_size, self.current_depth)?;
        
        self.active_streams += 1;
        info!("Started secure decompression stream (active: {})", self.active_streams);
    ...
    pub fn finish_stream(&mut self) {
        if self.active_streams > 0 {
            self.active_streams -= 1;
            info!("Finished secure decompression stream (active: {})", self.active_streams);
        }
    }
...
mod tests {
    ...
    fn test_compression_size_limit() {
        ...
        let compressor = SecureCompressor::new(detector, CompressionStrategy::Dictionary { 
