...
use crate::{
    ...
    domain::{
        ...
        ports::{StreamRepository, EventPublisher},
    ...
    P: EventPublisher + Send + Sync,
{
    async fn handle(&self, command: CreateStreamCommand) -> ApplicationResult<StreamId> {
        ...
        let mut session = self.repository
            .find_session(command.session_id).await
            .map_err(ApplicationError::Domain)?
        ...
        if let Some(config) = command.config {
            if let Some(stream) = session.get_stream_mut(stream_id) {
                stream.update_config(config).map_err(ApplicationError::Domain)?;
            }
        }
    ...
    P: EventPublisher + Send + Sync,
{
    async fn handle(&self, command: GenerateFramesCommand) -> ApplicationResult<Vec<Frame>> {
        ...
        // Get stream
        let stream = session.get_stream_mut(command.stream_id)
            .ok_or_else(|| ApplicationError::NotFound(
                format!("Stream {} not found", command.stream_id)
            ))?;
        
        // Generate frames
        let frames = stream.create_patch_frames(
            command.priority_threshold,
            command.max_frames,
        ).map_err(ApplicationError::Domain)?;
    ...
    P: EventPublisher + Send + Sync,
{
    async fn handle(&self, command: BatchGenerateFramesCommand) -> ApplicationResult<Vec<Frame>> {
        ...
        let frames = session.create_priority_frames(command.max_frames)
            .map_err(ApplicationError::Domain)?;
...
mod tests {
    ...
    impl StreamRepository for MockRepository {
        async fn find_session(&self, session_id: SessionId) -> crate::domain::DomainResult<Option<StreamSession>> {
    ...
    impl EventPublisher for MockEventPublisher {
        ...
        async fn publish_batch(&self, _events: Vec<DomainEvent>) -> crate::domain::DomainResult<()> {
            Ok(())
        }
    }
